...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <nav
        className={`fixed inset-y-0 left-0 z-[100] flex w-[210px] flex-col overflow-y-auto border-r border-border bg-surface transition-transform duration-200 md:transition-[width] ${
          collapsed ? "md:w-[52px]" : "md:w-[210px]"
      ...
      >
        ...
        <div className="border-b border-border px-[18px] pb-3.5 pt-5">
          <h1 className={`font-bold tracking-wide text-accent ${collapsed ? "md:text-sm" : "text-xl"}`}>
            <span className="md:hidden">Mi Panel</span>
            <span className="hidden md:inline">{collapsed ? "MP" : "Mi Panel"}</span>
          </h1>
          <span className={`mt-0.5 block text-[11px] text-text3 ${collapsed ? "md:hidden" : ""}`}>
            Calendario y Finanzas
          </span>
        </div>
        ...
        <div className="flex-1 py-1">
          {NAV.map((group, gi) => (
            <div key={gi}>
              {group.group && (
                <div
                  className={`mt-1.5 px-2.5 pb-0.5 pt-2 text-[10px] uppercase tracking-[2px] text-text3 ${
                    collapsed ? "md:hidden" : ""
                  }`}
                >
                  {group.group}

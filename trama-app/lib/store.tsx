...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    client
      ...
      .then(async ({ data: row, error }) => {
        ...
        if (row?.data) {
          ...
          await client.from("trama_app_state").upsert({ id: ROW_ID, data: seed, updated_at: new Date().toISOString() });
        }
    ...
    const channel = client
      .channel("trama_app_state_changes")
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "trama_app_state", filter: `id=eq.${ROW_ID}` },
        (payload) => {

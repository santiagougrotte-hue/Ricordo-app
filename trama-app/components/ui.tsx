...
type Color = "accent" | "green" | "red" | "blue" | "orange" | "purple" | "none";
...
export function KpiCard({
  ...
  return (
    <div className={`rounded-[10px] border border-border ${colorBorder[color]} border-t-2 bg-surface p-4`}>
      <div className="text-[10px] uppercase tracking-[1.2px] text-text3">{label}</div>
      <div className={`font-display my-1.5 text-2xl leading-none font-semibold ${colorValue[color]}`}>{value}</div>
...
const btnVariants: Record<BtnVariant, string> = {
  primary: "bg-accent text-[#fffdfa] hover:bg-accent2",
...
type BadgeColor = "green" | "red" | "orange" | "blue" | "purple" | "accent";
...
export function Badge({ color, children }: { color: BadgeColor; children: React.ReactNode }) {
  return (
    <span
      className={`inline-flex items-center whitespace-nowrap rounded-full px-2 py-0.5 text-[11px] font-medium ${badgeColors[color]}`}
    >
      {children}
    </span>
  );
...
export function PageHeader({
  ...
  return (
    <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 className="font-display text-[22px] font-semibold text-text">{title}</h2>
...
export function Td({
  ...
  return (
    <td
      onClick={onClick}
      className={`border-b border-border/60 px-3 py-2.5 align-middle ${main ? "font-medium text-text" : "text-text2"} ${onClick ? "cursor-pointer" : ""} ${className}`}
    >
      {children}
    </td>
  );
    ...
    <div className="mb-4 flex gap-4 border-b border-border">
      {options.map((o) => (
        <div
          key={o.value}
          onClick={() => onChange(o.value)}
          className={`cursor-pointer border-b-2 px-0.5 pb-2.5 text-[12.5px] transition-colors ${
            value === o.value ? "border-accent font-medium text-text" : "border-transparent text-text3 hover:text-text"
          }`}
        >
          {o.label}
        </div>
  ...
  return (
    <div className="flex gap-1.5">
      <div
        ...
        className={`cursor-pointer rounded-md border px-3 py-1.5 text-[12px] transition-colors ${
          value ? "border-accent/30 bg-accent-dim text-accent" : "border-border bg-surface2 text-text3"
        }`}
      >
        Sí
      </div>
    ...
    <div className="mb-6 flex flex-wrap items-center gap-1.5">
      {labels.map((label, i) => (
        <React.Fragment key={label}>
          ...
          {i < labels.length - 1 && <div className="h-px w-3 bg-border" />}
        </React.Fragment>
      ))}
    </div>
  );
